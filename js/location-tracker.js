...
if (DWELL_TIME) {
    rawData.forEach(function (data) {
        var dateTime = new Date(data.start);
        ...
        var endTime = parseTimeString(endMin, endHour)
        var shownTime = "Day " + day + ", " + timeString + " - " + endTime;
        ...
        if (area >= 0 && day > 0) {
            ...
            parsedNameList.add(data.name.toLowerCase());
        }
    });
} else {
    rawData.forEach(function (data) {
...
function iterateRecords() {
    ...
    if (DWELL_TIME) {
        parsedData.forEach(function (data) {
            if (data.id == search.value
                ...
                // attendeeId = data.id;
                areaCount[data.area].count += data.duration;
    ...
    if (DWELL_TIME) {
        $("#durationSpent").html("Total duration spent in most frequently visited area: <b>" + duration + " mins</b>");
        $("#totalDuration").html("Total duration spent in event: <b>" + totalDuration + " mins</b>");
    }
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        table.rows[i].onmouseleave = function () {
            if (!isPlaying) {
                ...
                if (!selectedMarker || counter != this.rowIndex) {
        ...
        table.rows[i].onclick = function () {
            if (!isPlaying) {
                mymap.closePopup(selectedMarker);
                table.rows[counter].style.background = "#ffffff";
                counter = this.rowIndex;
...
function addRow(data) {
    ...
    if (DWELL_TIME) {
        ...
        var textnode3 = document.createTextNode(data.duration);
...
function addHeader() {
    ...
    if (DWELL_TIME) {
        ...
        var textnode3 = document.createTextNode("Duration (in min)");
        cell3.appendChild(textnode3);
        row.appendChild(cell3);
    }
    row.id = "stickyHeader";
